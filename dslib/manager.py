from dsobj.bonfire import DSBonfire
from dsobj.item import DSItem, DSInfusion
from dslib.process import DSProcess, Stats
from dslib.cmd import DSCmd
from dsres.resources import SAVE_DIR, play_random_sound
from dsres.commands import nest_reset, nest_remove, nest_add
...
class DarkSouls(DSProcess):
    ...
    def __init__(self, debug=False):
        super(DarkSouls, self).__init__(DarkSouls.PROCESS_NAME, debug=debug)
        self._debug = debug
    ...
    def update_version(self):
        set_title("DarkShell — Game Version: %s" % self.get_version())
    ...
    def add_new_item(args):
        if len(args) == 0:
            ...
            if args[0] == "clear":
                ...
                if item_name is None or not item_name.strip():
                    return False
    ...
    def get_upgrade_value_pyro_flame(item: DSItem):
        is_pyro_asc = item.get_upgrade_type() == DSItem.Upgrade.PYRO_FLAME_ASCENDED
    ...
    def get_upgrade_value_armor_or_unique(item: DSItem):
        is_unique = item.get_upgrade_type() == DSItem.Upgrade.UNIQUE
    ...
    def print_status(self):
        ...
        for stat in self.stats.keys():
            if stat not in exclude:
                print(Fore.LIGHTBLUE_EX + ("\t%s:" % stat.value) +
    ...
    def raise_warp_error(b_full_name: str):
        from dsres.commands import DS_NEST
        if not b_full_name.strip() or b_full_name.lower() == DSCmd.default:
            raise ArgumentError("No arguments given!")
    ...
    def upgrade_item(self, i_name: str, i_count: int):
        if i_name not in self.items.keys():
            ...
            if item.get_upgrade_type() == DSItem.Upgrade.NONE:
                ...
                return
            elif item.get_upgrade_type() in (DSItem.Upgrade.ARMOR, DSItem.Upgrade.UNIQUE):
                upgrade = DarkSouls.get_upgrade_value_armor_or_unique(item)
                if upgrade is None:
                    return
                i_id += int(upgrade)
            elif item.get_upgrade_type() in (DSItem.Upgrade.PYRO_FLAME, DSItem.Upgrade.PYRO_FLAME_ASCENDED):
                upgrade = DarkSouls.get_upgrade_value_pyro_flame(item)
                if upgrade is None:
                    return
                i_id += int(upgrade) * 100
            elif item.get_upgrade_type() in (DSItem.Upgrade.INFUSABLE, DSItem.Upgrade.INFUSABLE_RESTRICTED):
                values = [
    ...
    def notify(self):
        p_type = self.get_phantom_type()
        for i in (0, 1, 2, 12) * 30:
            self.set_phantom_type(i)
            sleep(0.16)
        for _ in range(90):
            self.set_phantom_type(p_type)
            sleep(0.033)
            ...
            for file in item_files:
                ...
                for i in items[1:]:
                    try:
                        ...
                        print(
                            Fore.RED + (format_exc() if self._debug else
    ...
    def manage_thread_info(args):
        if len(args) == 0:
            ...
            from dsres.commands import DS_NEST
            arg = args[1:] if len(args) > 1 else DS_NEST["meta"]["processes"]["clear"].keys()
    ...
    def switch(self, command: str, arguments: list):
        ...
        class Switcher:
            ...
            @classmethod
            def switch(cls):
                case_name = DSCmd.get_method_name(prefix=command, name=arguments[0])
                default_func = getattr(cls, DSCmd.get_method_name(prefix=command, name="default"))
            ...
            def set_default():
                ...
                ds1.level_stat(stat_name, stat_value)
            ...
            def get_default():
                ...
                print(Fore.LIGHTBLUE_EX + ("FLAG %d:" % flag_id) + Fore.LIGHTYELLOW_EX +
                      (" %s" % (ds1.read_event_flag(flag_id))) + Fore.RESET)
            ...
            def enable_default():
                ...
                flag_id = convert(arguments[0], int)
                enable = arguments[1]
                ds1.write_event_flag(flag_id, enable)
            ...
            def set_ng():
                ...
                ds1.set_ng_mode(new_game)
                print(Fore.GREEN + ("NG changed to +%d" % new_game) + Fore.RESET)

            @staticmethod
            def set_covenant():
                covenant_name = arguments[1]
                ds1.validate_covenant(covenant_name)
                covenant_id = ds1.covenants[covenant_name]
                ds1.set_covenant(covenant_id)
                print(Fore.GREEN + ("Covenant changed to %s" % DarkSouls.get_name_from_arg(covenant_name)) + Fore.RESET)
            ...
            def get_last_animation():
                ds1.read_performed_animations()

            @staticmethod
            def enable_super_armor():
                enable = arguments[1]
                ds1.set_super_armor(enable)
            ...
            def enable_draw():
                enable = arguments[1]
                ds1.set_draw_enable(enable)
                print(Fore.GREEN + ("DRAW %s" % ("enabled" if enable else "disabled")) + Fore.RESET)
                DarkSouls.update_static_func(command, arguments, enable)

            @staticmethod
            def enable_gravity():
                enable = arguments[1]
                ds1.set_no_gravity(not enable)
            ...
            def enable_no_stamina_consume():
                enable = arguments[1]
                ds1.set_no_stamina_consume(enable)
            ...
            def enable_no_goods_consume():
                enable = arguments[1]
                ds1.set_no_goods_consume(enable)
            ...
            def enable_no_attack():
                enable = arguments[1]
                ds1.set_no_attack(enable)
                print(Fore.GREEN + ("NO ATTACK %s" % ("enabled" if enable else "disabled")) + Fore.RESET)
                DarkSouls.update_static_func(command, arguments, enable)

            @staticmethod
            def enable_no_move():
                enable = arguments[1]
                ds1.set_no_move(enable)
                print(Fore.GREEN + ("NO MOVE %s" % ("enabled" if enable else "disabled")) + Fore.RESET)
                DarkSouls.update_static_func(command, arguments, enable)

            @staticmethod
            def enable_no_damage():
                enable = arguments[1]
                ds1.set_no_damage(enable)
            ...
            def enable_no_ammo_consume_all():
                enable = arguments[1]
                ds1.set_no_ammo_consume_all(enable)
            ...
            def enable_player_dead_mode():
                enable = arguments[1]
                ds1.set_player_dead_mode(enable)
            ...
            def on_flag_notify():
                ...
                Thread(target=ds1.start_listen, args=(hash(evt), flag_id, state, evt)).start()
