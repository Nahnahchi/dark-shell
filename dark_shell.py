            ...
            if args[0] == "start":
                ...
                if i_name in self.items.keys():
                    ...
                    if item.get_upgrade_type() == Upgrade.NONE:
                        ...
                        return
                    elif item.get_upgrade_type() == Upgrade.ARMOR or item.get_upgrade_type() == Upgrade.UNIQUE:
                        ...
                        if upgrade is None:
                            return
                        elif int(upgrade) > max_upgrade or int(upgrade) < 0:
                            print("Can't upgrade %s to +%s" % ("Unique" if is_unique else "Armor", upgrade))
                            return
                        else:
                            i_id += int(upgrade)
                    elif item.get_upgrade_type() == Upgrade.PYRO_FLAME or \
                            item.get_upgrade_type() == Upgrade.PYRO_FLAME_ASCENDED:
                        ...
                        if upgrade is None:
                            ...
                            print("Can't upgrade %sPyromancy Flame to +%s" % ("Ascended " if is_pyro_asc else "",
                                                                              upgrade))
                            return
                        else:
                            i_id += int(upgrade) * 100
                    elif item.get_upgrade_type() == Upgrade.INFUSABLE or \
